...
Now decoupled from Yahoo Finance – expects data to be passed in.
"""
...
def _parse_fundamentals(r):
    """
    Helper to parse fundamentals from a generic dict structure
    IF you still have something shaped like Yahoo's quoteSummary.
    If not needed, you can delete this too.
    """
...
def calc_adx(h, l, c, n=14):
    ...
    def sm(a):
        s = [a[:n].sum()]
        [s.append(s[-1] - s[-1] / n + a[i]) for i in range(n, len(a))]
        return np.array(s)
...
def score_stock(fd, series):
    ...
    if above_200:
        L3 += 8; finds.append({"t": "p", "tx": "Price above 200-day EMA — long-term uptrend intact"})
    else:
        finds.append({"t": "n", "tx": "Price below 200-day EMA — long-term trend bearish"})
    if above_50:
        L3 += 5; finds.append({"t": "p", "tx": "Price above 50-day EMA — medium-term momentum positive"})
    if golden:
        L3 += 5; finds.append({"t": "p", "tx": "Golden Cross active (50>200 EMA) — bullish signal"})

    if 40 <= rsi <= 70:
        L3 += 4; finds.append({"t": "p", "tx": f"RSI {rsi:.1f} — healthy momentum zone"})
    elif rsi < 30:
        L3 += 3; finds.append({"t": "p", "tx": f"RSI {rsi:.1f} oversold — potential reversal"})
    elif rsi > 75:
        finds.append({"t": "n", "tx": f"RSI {rsi:.1f} overbought — correction likely"})

    if macd["hist"] > 0:
        L3 += 3; finds.append({"t": "p", "tx": "MACD histogram positive — bullish momentum"})
    else:
        finds.append({"t": "n", "tx": "MACD histogram negative — bearish pressure"})

    # ─ Ownership / Risk
    inst = safe(fd.get("held_institutions", 0)) * 100
    if inst > 50:
        L4 += 12; finds.append({"t": "p", "tx": f"High institutional ownership {inst:.1f}%"})
    elif inst > 25:
        L4 += 7
    elif inst > 10:
        L4 += 3
    ...
    if insiders > 40:
        L4 += 5; finds.append({"t": "p", "tx": f"High promoter/insider holding {insiders:.1f}%"})
    elif insiders > 25:
        L4 += 3
    elif insiders < 5:
        finds.append({"t": "n", "tx": f"Low insider holding {insiders:.1f}%"})

    beta = safe(fd.get("beta", 1), 1)
    if beta < 0.8:
        L4 += 3; finds.append({"t": "p", "tx": f"Low beta {beta:.2f} — defensive stock"})
    elif beta > 1.5:
